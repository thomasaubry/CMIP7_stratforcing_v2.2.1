...
%script are defined in the EVA_H paper. Boxes are indexed from 1 to 8,
%from South to North and from top to bottom.

%Given the boundaries of the boxes of our model (defined by h1lim, h2lim
%and latlim) and a set of SO2 injection source parameters (mass, height, etc)
%this function returns the mass of sulfur injected in each box of the model and the time of injection.


function [injec timelist]=so2injection_8boxes_cmip7(h1lim,h2lim,latlim,so2mass,eruheight,erulat,eruyear,erumonth,eruday)
...
%so2 mass input should be in Tg SO2, eruheight in km above sea level,
%erulat degree north, year calendar year. month is 1-12, day 1-31
...
erutropo=NaN(size(eruheight));%tropopause height in km a.s.l.
eruhstar=eruheight./erutropo;
erudate=[eruyear erumonth eruday];%date
erudate=(datenum(erudate)-datenum([1979 1 1 0 0 0]))*12/365.25; %date in number of months since 1979
timelist=erudate;
so2mass=0.50052*so2mass; %convert mass from Tg so2 to tg of S
...
for i=1:length(erudate)
    ...
    %volcano location
    if erulat(i)<-latlim & ~isnan(erutropo(i))
        SHtrop=erutropo(i);
    elseif erulat(i)>latlim & ~isnan(erutropo(i))
        NHtrop=erutropo(i);
    elseif  ~isnan(erutropo(i))
        Ttrop=erutropo(i);
    end

    %assume stratospheric injection if before 1979
    if eruyear(i)<1979
        Ttrop=min(eruheight(i)-3*dh,Ttrop);
        SHtrop=min(eruheight(i)-3*dh,SHtrop);
        NHtrop=min(eruheight(i)-3*dh,NHtrop);
    end
