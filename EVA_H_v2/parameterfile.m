...
%script are defined in the EVA_H original paper. Boxes are indexed from 1 to 8,
%from South to North and from top to bottom. Default model parameter
%values proposed in this script enable to best reproduce the 1979-2023
%CMIP7 stratospheric aerosol optical properties from the 1979-2023
%CMIP7 volcanic SO2 emission inventory. See the CMIP7 associated papers for
%details.

%Note parameter values have changed between v1 and v2 of EVA_H
...
%==========================================================================
%SULFATE PRODUCTION TIMESCALES, in month
%==========================================================================

tauprod = [0.0115;9.81;0.311;NaN(5,1)];

%For simplicity, we left this parameter as a vector with 8 values,
%previously corresponding to 1 value per box. However, the production
%timescale is now (v2) equal to tauprod(1).*(1+tauprod(2).*[SO2 mass]).*[SO2 mass].^tauprod(3)
%and it is the same in all boxes. This means tauprod(1:3) are parameters
%used in the equation above, and the rest are just unused NaN values.
...
tauloss = [1.94;18.8;1.94;2.68;57.7;2.68;4.5;4.5];
...
%model performance, as defined in EVA_H original paper. We thus do not set the
...
%volcanic injections
%NOTE: these are not used in EVA_H v2; leaving line below but now
%irrelevant
